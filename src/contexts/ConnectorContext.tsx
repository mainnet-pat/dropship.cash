...
export class WalletConnect2Connector implements IConnector {
  ...
  async onSessionConnected(_session: SessionTypes.Struct) {
    ...
    this.chains = USE_CHIPNET ? ["bch:bchtest"] : ["bch:bitcoincash"];
  ...
  async _subscribeToEvents(_client: Client) {
    ...
    _client.on("proposal_expire", (args) => {
      DEFAULT_LOGGER && console.log("EVENT", "proposal_expire", args);
    ...
    try {
      const requiredNamespaces = {
        "bch": {
            ...
            "chains": [
                USE_CHIPNET ? "bch:bchtest" : "bch:bitcoincash"
            ],
            "events": [
                "addressesChanged"
            ]
...
export function ConnectorContextProvider({
  ...
  }, [setConnector, setConnectorType, connectorType, setActive, active]);
  ...
  const address = useCallback(() => {
    return new Promise<string | undefined>((resolve) => resolve(connector?.address()));
  }, [connector]);
